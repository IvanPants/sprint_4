...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	var spentCalories float64 = (float64(runningCaloriesMeanSpeedMultiplier) * meanSpeed(action, duration) *
		runningCaloriesMeanSpeedShift) * weight / float64(mInKm) * duration * float64(minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	var meanSpeedingMsecSqrt float64 = math.Pow(meanSpeed(action, duration)*kmhInMsec, 2.0)
	var spentCalories float64 = (walkingCaloriesWeightMultiplier*weight + (meanSpeedingMsecSqrt/(height/float64(cmInM)))*
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	var spentCalories float64 = (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) *
